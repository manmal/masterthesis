...
As has been described in \cite{gansner:1998}, the application of pseudo-physical forces on an undirected graph provides for a improvement of the graph layout. This is achieved by adding attractive or repulsive forces to all nodes in the graph, such that nodes push away from or attract each other. As long as there is energy left in a graph (i.e., there are objects which are not in their optimal position), the nodes are moved in a way that satisfies the applied forces. Such a force can be compared to a spring connecting two nodes, and for this reason a subset of those graph layouts are called \textbf{spring model}, on which has been elaborated in the previous subchapter. Obviously, spring model graphs in the context of multidimensional scaling are very tightly related with the computational aspects of MDS. Still, there are visualization-only aspects to spring graphs which can be formulated as the following questions:

\begin{itemize}
	\item How are edges displayed, or are they displayed at all?
...
TODO: read and integrate \cite{Muelder:2010fk}.
